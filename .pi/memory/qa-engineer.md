...
- **All catch-handler `console.error` calls replaced with `logInbox` (pit2-1rnz.17/18):** `logInbox(cwd, message)` defined in `utils.ts`, imported in both `index.ts` and `broker.ts`. Only remaining `console.error` is at `index.ts:856` inside `scheduleInboxPing` retry logic — not a catch handler, pre-existing and acceptable. Any new `console.error` inside a `.catch()` handler is a regression.
