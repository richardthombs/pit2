...
- **pit2-tzr run (post-all-fixes baseline):** 4 concurrent tasks, no cross-agent contamination observed. newSession() per-task and epic-level context-injection limits both confirmed working. close_reason dirty on 3/4 due to response formatting (see note below), but words themselves correct.
...
- **close_reason formatting sensitivity:** Whether `close_reason` captures just the word or the full sentence depends on how the agent formats its response. If the agent puts the word on its own paragraph (`**word**\n\nSentence…`), `captureResult` captures only the first paragraph → clean single-word `close_reason`. If the agent puts word and sentence on the same line (`**Word.** Sentence…`), the entire line becomes `close_reason`. Observed in pit2-tzr: architect (clean) vs pi-specialist/prompt-engineer/typescript-engineer (dirty). Not a code bug — a response-formatting issue.
